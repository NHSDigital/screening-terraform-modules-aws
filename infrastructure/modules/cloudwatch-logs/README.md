...
| Encryption | Optional KMS key support; AWS-managed by default |
| Creation gate | Both log group and stream gated via `module.this.enabled` |
...
- `create_log_stream = true` requires `create_log_group = true`; the check block enforces this.
...
| <a name="input_kms_key_id"></a> [kms\_key\_id](#input\_kms\_key\_id) | ARN of KMS key for log group encryption. When null, uses AWS-managed encryption. | `string` | `null` | no |
