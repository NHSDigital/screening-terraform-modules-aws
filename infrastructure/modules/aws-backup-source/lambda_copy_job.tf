...
resource "aws_iam_role" "iam_for_lambda_copy_job" {
  ...
  name               = "${var.name_prefix}-backup-cross-account-copy-job-lambda"
...
resource "aws_iam_role_policy" "cross_account_iam_permissions" {
  count  = var.backup_plan_config_rds.enable ? 1 : 0
  name   = "${var.name_prefix}-cross-account-copy-job-lambda"
...
data "archive_file" "start_cross_account_copy_job_lambda_zip" {
  type        = "zip"
  source_dir  = "${path.module}/../../lambdas/cross-account-copy-job/resources"
...
resource "aws_lambda_function" "start_cross_account_copy_job_lambda" {
  ...
  function_name    = "${var.name_prefix}-backup-start-cross-account-copy-job"
...
resource "aws_lambda_permission" "allow_eventbridge" {
  ...
  source_arn    = "arn:aws:events:eu-west-2:${data.aws_caller_identity.current.account_id}:rule/${var.name_prefix}-backup-start-cross-account-copy-job-rule"
}
