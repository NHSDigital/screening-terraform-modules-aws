...
variable "source_account_ids" {
  # These source account IDs are used in the policy to allow root in each source account
...
variable "vault_lock_min_retention_days" {
  description = "The minimum retention period required on recovery points when vault lock enabled"
  type        = number
  default     = 365
}

variable "vault_lock_max_retention_days" {
  description = "The maximum retention period required on recovery points when vault lock enabled"
  type        = number
  default     = 365
}
